...
<body>
	...
	<script type="text/javascript">
		var table = $('#table').bootstrapTable({
			url : ctx + "/carsystem/table",
			searchPlaceholder : "车系名称",
			search : true,
			toolbar : '#toolbar',
			columns : [ {
				checkbox : true
		...
		$('#deleteBtn').click(function() {
			...
			parent.$.smkConfirm({
				...
				if (res) {
					$.post(ctx + '/carsystem/delete', {id:st[0].id}, function(data){
						if(data.success){
		...
		$('#addBtn').click(function() {
			window.top.layer.open({
				...
				yes : function(index, layero) {
					...
					$.post(ctx + '/carsystem/add', layero.find('#layui-layer-iframe' + index).contents().find('#myForm').serialize(), function(data) {
						if (data.success) {
		...
		$('#editBtn').click(function() {
			...
			window.top.layer.open({
				...
				shadeClose : false, //点击遮罩关闭层
				area : [ '800px', '320px' ],
				content : ctx + '/carsystem/editPage',
				btnAlign : 'c',
				btn : [ '确定', "关闭" ],
				yes : function(index, layero) {
					if(!layero.find('#layui-layer-iframe' + index).contents()[0].defaultView.$('#myForm').valid()){
					...
					$.post(ctx + '/carsystem/edit', layero.find('#layui-layer-iframe' + index).contents().find('#myForm').serialize(), function(data) {
						if (data.success) {
							window.top.layer.closeAll();
							table.bootstrapTable('refresh');
						} else {
				...
				success : function(layero, index) {
					mk.load(layero.find('#layui-layer-iframe' + index).contents().find('#myForm'), ctx + '/carsystem/get?id=' + st[0].id);
				}
			});
		});
	</script>
