	...
	<script type="text/javascript">
		...
		$('#deleteBtn').click(function() {
			...
			parent.$.smkConfirm({
				...
				if (res) {
					$.post(ctx + '/tag/delete', {id:st[0].id}, function(data){
						if(data.success){
		...
		$('#addBtn').click(function() {
			window.top.layer.open({
				...
				yes : function(index, layero) {
					...
					$.post(ctx + '/tag/add', layero.find('#layui-layer-iframe' + index).contents().find('#myForm').serialize(), function(data) {
						if (data.success) {
		...
		$('#editBtn').click(function() {
			...
			window.top.layer.open({
				...
				yes : function(index, layero) {
					...
					$.post(ctx + '/tag/edit', layero.find('#layui-layer-iframe' + index).contents().find('#myForm').serialize(), function(data) {
						if (data.success) {
							window.top.layer.closeAll();
							table.bootstrapTable('refresh');
						} else {
				...
				success : function(layero, index) {
					mk.load(layero.find('#layui-layer-iframe' + index).contents().find('#myForm'), ctx + '/tag/get?id=' + st[0].id);
				}
			});
		});
	</script>
